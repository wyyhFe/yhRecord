...
| `record/` | Spring Boot 3.5 + Java 21 + MyBatis-Plus + Spring Security + JWT + Redis + Spring AI | `./mvnw.cmd spring-boot:run` |
...
```
仅有 `RecordApplicationTests.java` 一个空测试类。`spring-boot-starter-test` + `spring-security-test` 依赖在，但未写任何真实测试。不要假设有测试覆盖。
...
```
构建后会自动执行 `scripts/sanitize-mp-output.mjs` 清理产物。
...
| `pages/checkin/` | 打卡任务 |
| `pages/memorial/` / `memorial-manage/` / `memorialPage/` | 纪念日 |
| `pages/memory/` | 那年今日 |
| `pages/auth/` | 登录 |
...
### 构建/配置
- **esbuild target 必须是 `es2017`**（`vite.config.ts`）—— 微信小程序 JS 引擎不支持 ES2020 可选链（`?.`）和空值合并（`??`）。绝不能提升此 target。
- **UnoCSS 快捷类 `page-shell-safe`** = `min-h-screen box-border px-6 pb-40 pt-6`。这是所有页面的根容器样式标准。
- **无 CI** —— 仓库不存在 `.github/` 目录或 CI 流水线。
...
- 向量库用 Milvus（开发环境：`docker-compose -f docker-compose.dev.yml up -d`）
